...
public class NPCUpdateTask implements Task {
	...
	public void execute(GameEngine context) {
		...
		for (Iterator<NPC> it$ = player.getLocalNPCs().iterator(); it$
			...
			if (World.getWorld().getNPCs().contains(npc)
					&& !npc.isTeleporting() && !npc.isInvisible()
		 ...
		 */
		for (NPC npc : RegionManager.getLocalNpcs(player.getLocation())) {
			/*
			 ...
			 * If they should not be added ignore them.
			 */
			if(npc == null || player.getLocalNPCs().contains(npc) || npc.isInvisible() ||
					!npc.getLocation().isWithinDistance(player.getLocation())) {
				continue;
			}
	...
	private void updateNPC(PacketBuilder packet, NPC npc) {
		...
		if (flags.get(UpdateFlag.FORCED_CHAT)) {
			packet.putRS2String(npc.getForcedText());
		}
		if (flags.get(UpdateFlag.HIT_2)) {
		...
		if (flags.get(UpdateFlag.TRANSFORM)) {
			if (npc.getTransformId() != -1)
				packet.putLEShortA(npc.getTransformId());
		}
		if (flags.get(UpdateFlag.FACE_COORDINATE)) {
	...
	private static void appendHitUpdate(final NPC npc,
			final PacketBuilder updateBlock) {
		updateBlock.putByteA((byte) npc.getDamage().getHitDamage1());
		updateBlock.putByteC((byte) npc.getDamage().getHitType1());
		updateBlock.putByteA((byte) getCurrentHP(npc.getHealth(), npc.getDefinition().getMaxHealth(), 100));
		updateBlock.put((byte) 100);
	}

	private static void appendHit2Update(final NPC npc,
			final PacketBuilder updateBlock) {
		updateBlock.putByteC((byte) npc.getDamage().getHitDamage2());
		updateBlock.putByteS((byte) npc.getDamage().getHitType2());
		updateBlock.putByteS((byte) getCurrentHP(npc.getHealth(), npc.getDefinition().getMaxHealth(), 100));
