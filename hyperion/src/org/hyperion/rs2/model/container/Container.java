...
public class Container {
		...
		if(item.getDefinition().isStackable() || type.equals(Type.ALWAYS_STACK)) {
			for(int i = 0; i < items.length; i++) {
				if(items[i] != null && items[i].getId() == item.getId()
						&& items[i].getDegrade() == item.getDegrade()) {
					long totalCount = item.getCount() + items[i].getCount();
					if(totalCount >= Constants.MAX_ITEMS || totalCount < 1) {
						if(freeSlot() == -1) return false;
						int extraCount = (int) (totalCount - Constants.MAX_ITEMS);
						item = new Item(item.getId(), extraCount, item.getDegrade());
						set(i, new Item(items[i].getId(), Constants.MAX_ITEMS, items[i].getDegrade()));
						continue;
					}
					set(i, new Item(items[i].getId(), item.getCount() + items[i].getCount(), item.getDegrade()));
					return true;
				}
			}
			int slot = freeSlot();
			...
			if(slots >= item.getCount()) {
				...
				try {
					for(int i = 0; i < item.getCount(); i++) {
						set(freeSlot(), new Item(item.getId(), 1, item.getDegrade()));
					}
					if(b) {
						fireItemsChanged();
					}
	 ...
	 */
	public int getSlotById(Item id) {
		for(int i = 0; i < items.length; i++) {
			if(items[i] == null) {
				continue;
			}
			if(items[i].getId() == id.getId() && items[i].getDegrade() == id.getDegrade()) {
				return i;
			}
		}
		return -1;
	...
	public int getSlotById(int id) {
		for(int i = 0; i < items.length; i++) {
			...
			if(items[i].getId() == id && items[i].getDegrade() == 0) {
				return i;
			}
		}
		return -1;
	...
	public void set(int index, Item item) {
		if(capacity <= 28) {//This just makes it so bank isnt included.
			Item old = items[index];
			int toAdd = 0, toRemove = 0;
			if(old == null) {
				toAdd = (int) (item.getCount() * item.getDefinition().getWeight());
				weight += toAdd;
			} else if(item == null) {
				toRemove = (int) (old.getCount() * old.getDefinition().getWeight());
				weight -= toRemove;
			} else {
				toAdd = (int) (item.getCount() * item.getDefinition().getWeight());
				toRemove = (int) (old.getCount() * old.getDefinition().getWeight());
				weight = (weight + (toRemove - toAdd));
			}
		}
		items[index] = item;
		if(firingEvents) {
			...
			for(int i = 0; i < item.getCount(); i++) {
				...
				if(i == 0 && preferredSlot != -1) {
					...
					if(inSlot.getId() == item.getId() 
							&& inSlot.getDegrade() == item.getDegrade()) {
						slot = preferredSlot;
					}
				}
				if(slot != -1) {
	 ...
	 * @return The amount.
	 */
	public int getCount(Item i2) {
		int total = 0;
		for(int i = 0; i < items.length; i++) {
			if(items[i] != null) {
				if(items[i].getId() == i2.getId() && (total + items[i].getCount() <= Constants.MAX_ITEMS)
						&& items[i].getDegrade() == i2.getDegrade()) {
					total += items[i].getCount();
				}
			}
		}
		if(total < 0 || total >= Constants.MAX_ITEMS)
