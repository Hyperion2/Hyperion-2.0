...
public class Bank {
	...
	public static void withdraw(Player player, int slot, int id, int amount) {
		...
		int transferAmount = player.getBank().getCount(item);
		...
		if(player.getSettings().isWithdrawingAsNotes()) {
			if(item.getDefinition().isNoteable()) {
				newId = item.getDefinition().getNoteCounterPart();
			}
		}
		ItemDefinition def = Definitions.forId(newId);
		if(def.isStackable()) {
		...
		// now add it to inv
		if(player.getInventory().add(new Item(newId, transferAmount, item.getDegrade()))) {
	...
	public static void deposit(Player player, int slot, int id, int amount) {
		...
		try {
			...
			int transferAmount = player.getInventory().getCount(item);
			if(transferAmount >= amount) {
				transferAmount = amount;
			} else if(transferAmount == 0) {
				return; // invalid packet, or client out of sync
			...
			if(item.getDefinition().isStackable() || noted) {
				int bankedId = item.getDefinition().isNoted() ? item.getDefinition().getNoteCounterPart() : item.getId();
				...
				if(newInventoryAmount <= 0) {
					...
					newItem = new Item(item.getId(), newInventoryAmount, item.getDegrade());
				}
				if(!player.getBank().add(new Item(bankedId, transferAmount, item.getDegrade()))) {
					player.getActionSender().sendMessage("You don't have enough s1pace in your bank account."); // this is the real message
				} else {
				...
				if(!player.getBank().add(item)) {
					...
					for(int i = 0; i < transferAmount; i++) {
						...
						player.getInventory().set(player.getInventory().getSlotById(item), null);
						// }
					}
					player.getInventory().fireItemsChanged();
				}
