...
public class InterfaceState {
	...
	public void interfaceClosed() {
		...
		for(ContainerListener l : containerListeners) {
			...
			player.getOther().removeListener(l);
		}
	...
	public void closeEnterAmountInterface(int amount) {
		try {
			...
			case Trade.PLAYER_INVENTORY_INTERFACE:
				Trade.addItemToOffer(player, enterAmountSlot, amount);
				break;
				
			case Trade.TRADE_INTERFACE:
				Trade.removeItemFromOffer(player, enterAmountSlot, amount);
				break;
			}
		} finally {
			enterAmountInterfaceId = -1;
	...
	/**
	 * @param interfaceItem the interfaceItem to set
	 */
	public void setInterfaceItem(int interfaceItem) {
